(() => {
  ...
  style.textContent = `.orbi9-product-card{display:flex;flex-direction:column;overflow:hidden;max-width:300px;position:relative}.section#shop .grid{grid-template-columns:repeat(4,minmax(0,1fr));align-items:stretch;grid-auto-rows:360px}.section#shop .orbi9-product-card{width:100%;max-width:none;height:360px;min-height:360px;max-height:360px;box-sizing:border-box}.orbi9-product-card>a{display:flex;flex:1;flex-direction:column;min-height:0}.orbi9-product-card img{height:190px;min-height:190px;max-height:190px;aspect-ratio:auto}.orbi9-product-card .product-copy{display:flex;flex:1;flex-direction:column;min-height:0}.orbi9-product-card .product-copy h3{display:-webkit-box;flex:0 0 auto;margin:0 0 24px;min-height:2.6em;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:2}.orbi9-product-card .product-link{margin-top:auto}.orbi9-product-card .quick-add{margin:10px 12px 12px;padding:8px 11px;min-height:0;width:auto;align-self:flex-start;font-size:11px;line-height:1.15;white-space:nowrap}.category-card>a{display:block;color:inherit;text-decoration:none}.category-card img{display:block;width:min(100%,200px);height:200px;margin:0 auto;object-fit:cover}@media(max-width:900px){.section#shop .grid{grid-template-columns:repeat(2,minmax(0,1fr));grid-auto-rows:360px}.section#shop .orbi9-product-card{height:360px;min-height:360px;max-height:360px}.orbi9-product-card img{height:190px;min-height:190px;max-height:190px}}@media(max-width:600px){.section#shop .grid{grid-template-columns:1fr;grid-auto-rows:360px}}.orbi9-product-card.sold{cursor:not-allowed}.sold-banner{position:absolute;top:18px;left:-34px;z-index:2;width:170px;padding:8px 0;transform:rotate(-45deg);text-align:center;background:#d94a56;color:#fff;font:800 14px Oswald;letter-spacing:3px;box-shadow:0 3px 10px #0008}.orbi9-product-card img{width:100%;aspect-ratio:1;object-fit:contain;background:#111}.orbi9-product-card .product-copy{padding:18px}.orbi9-product-card a{color:inherit;text-decoration:none}.orbi9-product-card .product-link{margin-top:14px;text-decoration:underline}.catalog-search{position:relative}.search-preview{position:absolute;z-index:2147483646;top:calc(100% + 6px);left:0;right:0;background:#0b1622;border:1px solid #18cfff;box-shadow:0 12px 30px #000b}.search-preview a{display:flex;align-items:center;gap:12px;padding:10px 14px;color:#e9fbff;text-decoration:none}.search-preview a:hover,.search-preview a:focus{background:#123149}.search-preview img{width:42px;height:42px;object-fit:cover;background:#071326}.orbi9-detail{max-width:1100px;margin:60px auto;padding:0 24px;display:grid;grid-template-columns:minmax(0,1fr) minmax(280px, .8fr);gap:48px}.orbi9-detail img{width:100%;max-height:620px;object-fit:contain;background:#111}.orbi9-detail-copy{align-self:center}.orbi9-detail-copy h1{font-size:clamp(32px,5vw,70px);margin:10px 0}.orbi9-detail-copy .detail-price{font-size:26px;margin:24px 0}.orbi9-detail-copy .detail-category{letter-spacing:.16em;text-transform:uppercase}.orbi9-back{display:inline-block;margin-top:30px;text-decoration:underline}.detail-actions{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-top:20px}.detail-actions .detail-add-cart,.detail-actions .orbi9-back{margin-top:0}.detail-actions .orbi9-back{padding:11px 18px;border:1px solid #18cfff;background:#102538;color:#7de8ff;text-decoration:none;font:14px Oswald;letter-spacing:2px}.detail-actions .orbi9-back:hover{background:#18cfff;color:#031018}.quick-add{margin:0 18px 18px;cursor:pointer}.quantity-label{display:flex;align-items:center;gap:12px;margin:18px 0}.detail-quantity{width:72px;padding:8px;background:#102538;color:#e9fbff;border:1px solid #56d9ff}.stock-status{color:#8db7c9}@media(max-width:1000px){.section#shop .grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:500px){.section#shop .grid{grid-template-columns:1fr}}@media(max-width:700px){.orbi9-detail{display:block;margin-top:30px}.orbi9-detail-copy{padding:25px 0}}`;
  ...
  const addToCart = (product, requestedQty = 1) => {
    ...
    localStorage.setItem(key, JSON.stringify(cart)); window.dispatchEvent(new Event('orbi9-cart-updated')); window.dispatchEvent(new Event('orbi9-cart-added')); return true;
  };
  const card = p => { const stock = stockLimit(p); return `<article class="card orbi9-product-card${stock === 0 ? ' sold' : ''}" title="${stock === 0 ? 'Sold — this item is no longer available' : ''}">${stock === 0 ? '<span class="sold-banner" aria-label="Sold">SOLD</span>' : ''}<a href="?product=${encodeURIComponent(p.slug || slugify(p.name))}"><img src="${esc(productImage(p))}" alt="${esc(p.name)}"><div class="product-copy"><h3>${esc(p.name)}</h3></div></a><button class="btn quick-add" type="button" data-add-cart="${esc(p.id)}" ${stock === 0 ? 'disabled' : ''}>${stock === 0 ? 'OUT OF STOCK' : 'ADD TO CART'}</button></article>`; };
  ...
  const renderDetail = product => {
    ...
    detail.innerHTML = `<div class="orbi9-detail-media"><div class="orbi9-detail-hero"><button class="gallery-arrow gallery-prev" type="button" aria-label="Previous image">‹</button><img class="detail-gallery-main" src="${esc(gallery[galleryIndex])}" alt="${esc(product.name)}"><button class="gallery-arrow gallery-next" type="button" aria-label="Next image">›</button></div><div class="detail-gallery-thumbs" role="list">${gallery.map((src,i)=>`<button class="gallery-thumb ${i===galleryIndex?'active':''}" type="button" data-gallery-index="${i}" aria-label="View image ${i+1}"><img src="${esc(src)}" alt=""></button>`).join('')}</div><div class="gallery-count">Image <span class="gallery-current">${galleryIndex+1}</span> of ${gallery.length}</div></div><div class="orbi9-detail-copy"><div class="detail-category">${esc(product.category)}</div><h1>${esc(product.name)}</h1><div class="detail-price">$${Number(product.price || 0).toFixed(2)}</div><p>${esc(product.description || 'A considered object from the Orbi9 catalogue.')}</p><p class="stock-status">${canAdd ? (Number.isFinite(stock) ? `${stock} in stock` : 'In stock') : 'Out of stock'}</p>${quantityControl}<div class="detail-actions"><button class="btn detail-add-cart" type="button" ${canAdd ? '' : 'disabled'}>${canAdd ? 'ADD TO CART' : 'OUT OF STOCK'}</button><a class="orbi9-back" href="/">BACK</a></div></div>`;
  ...
  const priceSlider = document.querySelector('#shop-price-slider');
  ...
  if (priceSlider) { priceSlider.max = String(Math.ceil(actualMaxPrice)); priceSlider.value = String(Math.ceil(actualMaxPrice)); }
  ...
  if (shopGrid) {
    ...
    searchInput?.addEventListener('input', renderSearchResults); categoryFilter?.addEventListener('change', renderSearchResults); minPriceFilter?.addEventListener('input', renderSearchResults); priceFilter?.addEventListener('input', () => { if (priceSlider && priceFilter.value) priceSlider.value = priceFilter.value; renderSearchResults(); }); priceSlider?.addEventListener('input', () => { if (priceFilter) priceFilter.value = priceSlider.value; renderSearchResults(); }); document.querySelector('#shop-filter-reset')?.addEventListener('click', () => { if (categoryFilter) categoryFilter.value = ''; if (minPriceFilter) minPriceFilter.value = ''; if (priceFilter) priceFilter.value = ''; if (priceSlider) priceSlider.value = priceSlider.max; renderSearchResults(); });
