(() => {
  ...
  const card = p => `<article class="card orbi9-product-card"><a href="?product=${encodeURIComponent(p.slug || slugify(p.name))}"><img src="${esc(productImage(p))}" alt="${esc(p.name)}"><div class="product-copy"><div class="eyebrow">${esc(p.category)}</div><h3>${esc(p.name)}</h3>${p.subtitle ? `<p class="product-summary">${esc(p.subtitle)}</p>` : ''}<div class="price">$${Number(p.price || 0).toFixed(2)}</div><span class="product-link">View product</span></div></a></article>`;
