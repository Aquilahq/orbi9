(() => {
  ...
  const renderDetail = product => {
    ...
    detail.innerHTML = `<div><img src="${esc(productImage(product))}" alt="${esc(product.name)}"></div><div class="orbi9-detail-copy"><div class="detail-category">${esc(product.category)}</div><h1>${esc(product.name)}</h1><div class="detail-price">$${Number(product.price || 0).toFixed(2)}</div><p>${esc(product.description || 'A considered object from the Orbi9 catalogue.')}</p><button class="btn detail-add-cart" type="button">ADD TO CART</button><a class="orbi9-back" href="/">Back to catalogue</a></div>`;
